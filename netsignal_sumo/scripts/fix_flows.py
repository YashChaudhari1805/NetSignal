...
fix_flows.py
============
Regenerates ``flows/flow.xml`` and ``config/grid_nodes.txt`` by reading
real edge and junction IDs directly from the compiled network file.

Run this once after ``generate_network.py`` whenever the network changes.

Usage:
    python scripts/fix_flows.py

Requires:
    grid6x6.net.xml to exist in ``network/`` (generated by generate_network.py).
"""
...
def _parse_edges(root: ET.Element) -> dict[tuple[str, str], str]:
    """
    Extract a mapping of (from_node, to_node) -> edge_id from the network.

    Internal edges (IDs starting with ':') are excluded.
    """
    return {
...
def _parse_grid(root: ET.Element) -> dict[tuple[int, int], str]:
    """
    Build a (col, row) -> junction_id grid from traffic-light junctions.

    Junction coordinates are clustered into the nearest grid cell using
    equal-spaced bucketing along each axis.
    """
    junctions = [
    ...
    def _cluster(values: list[float], n: int) -> dict[float, int]:
...
def _build_routes(
    ...
    """
    Construct straight-line routes across the full grid in all four directions
    plus the emergency vehicle route from (0,0) to (3,3).

    Args:
        grid:          (col, row) -> junction ID mapping.
        edge_by_nodes: (from_id, to_id) -> edge ID mapping.

    Returns:
        Mapping of route ID to space-separated edge ID string.
    """
    G = _GRID_SIZE

    def edge(c1: int, r1: int, c2: int, r2: int) -> str | None:
        n1 = grid.get((c1, r1))
        n2 = grid.get((c2, r2))
        if not n1 or not n2:
            return None
    ...
    for col in range(G):
        ...
        if all(segs):
            ...
            if all(segs_rev):
                ...
                print(f"  WARNING: column {col} N-S route incomplete: {segs}")
    ...
    for row in range(G):
        ...
        if all(segs):
            ...
            if all(segs_rev):
                ...
                print(f"  WARNING: row {row} E-W route incomplete: {segs}")
...
def _build_flow_xml(routes: dict[str, str]) -> list[str]:
    """
    Generate the full content of flow.xml as a list of lines.

    Args:
        routes: Mapping of route ID to edge string.

    Returns:
        List of XML text lines ready to join and write.
    """
    G = _GRID_SIZE
    lines = [
    ...
    for col in range(G):
        for direction in ("NS", "SN"):
            route_id = f"r_{direction}_{col}"
            if route_id not in routes:
                continue
            lines.append(
                f'    <flow id="f{flow_id}" type="car" route="{route_id}"'
...
def _write_node_map(grid: dict[tuple[int, int], str]) -> None:
    """
    Write grid_nodes.txt with junction ID assignments for each (col, row) cell.
...
def main() -> None:
    """Parse the network, build routes and node map, write output files."""
